...
class JniIdentityTest {
    ...
    fun identityHasAllTypedFieldsAndTheCurrentAbi() {
        // The pin is deliberate: an ABI bump nobody wrote down must fail here. 2.4 is the S2.a
        // window (the two hardware-decode funnels); move this with KITECODEC_C_ABI_MINOR only.
        val identity = FFmpeg.identity
        assertTrue(identity.isAcceptable, identity.describe())
        assertEquals("2.4", identity.cAbiVersion)
        assertEquals(
